    ...
    <section class="bg-light position-relative overflow-hidden">
        ...
        <div class="container position-relative">
            ...
            <div class="row mb-4">
                <div class="col-12">
                    <h2 class="fs-1 fw-bold">
                        <span class="position-relative z-index-9">Rawat Inap</span>
                        <span class="position-relative z-index-1">RSHP
                            <!-- SVG START -->
                    ...
                    <p class="mb-0">Kenyamanan dalam perawatan dengan pilihan kamar Rumah Sakit Harapan PAPA.</p>
                </div>
            </div>
            ...
            <ul class="nav nav-pills nav-pill-soft mb-3" id="course-pills-tab" role="tablist">
                ...
                $rawatinap = mysqli_query($conn, "SELECT * FROM tbl_rawatinap WHERE admission = 1");

                while ($row = mysqli_fetch_assoc($rawatinap)) {
                ?>
                    <!-- Tab item -->
                    <li class="nav-item me-2 me-sm-5" role="presentation">
                        <button class="nav-link <?= ($row['id_rawatinap'] == 4 ? 'active' : '') ?>" id="course-pills-tab-<?= $row['id_rawatinap']; ?>" data-bs-toggle="pill" data-bs-target="#course-pills-tab<?= $row['id_rawatinap']; ?>" type="button" role="tab" aria-controls="course-pills-tab<?= $row['id_rawatinap']; ?>" aria-selected="true"><?= $row['nama_kamar']; ?></button>
                    </li>
                <?php } ?>
            </ul>
            <!-- Outer tabs END -->
            ...
            <div class="tab-content mb-0" id="course-pills-tabContent">
                ...
                ?>
                    <!-- Outer content START -->
                    <div class="tab-pane fade show  <?= ($row['id_rawatinap'] == 4 ? 'active' : '') ?>" id="course-pills-tab<?= $row['id_rawatinap']; ?>" role="tabpanel" aria-labelledby="course-pills-tab-<?= $row['id_rawatinap']; ?>">
                        <div class="row">
                            <!-- Left content START -->
                            <div class="col-lg-6">
                                <!-- Title -->
                                <h3><?= $row['nama_kamar']; ?></h3>
                                <h6 class="mt-4">Fasilitas Kamar :</h6>
                                <ul class="list-group list-group-borderless mb-3">
                                    <?php
                                    $kamar = explode(',', $row['fasilitas_kamar']);
                                    for ($i = 0; $i < count($kamar); $i++) {
                                        echo "<li class='list-group-item h6 fw-light d-flex mb-0'><i class='bi bi-patch-check-fill text-success me-2'></i>" . $kamar[$i] . "</li>";
                                    }
                                    ?>
                                </ul>
                            </div>
                            ...
                            <div class="col-lg-6">
                                ...
                                <div class="card p-2 shadow">
                                    <div class="overflow-hidden h-xl-300px">
                                        <img src="../assets/images/upload/rawatinap/<?= $row["kamar_picture"]; ?>" class="card-img-top" alt="course image">
                                    </div>
                                    <!-- Button Pesan Sekarang -->
                                    <div class="d-flex justify-content-center align-items-center">
                                        <?php
                                        if ($_SESSION['role'] == "") {
                                            echo '<a href="sign-up.php" class="btn btn-sm btn-primary-soft mb-0 my-0">Sign up</a>';
                                        } else {
                                            echo '<a href="' . $row['link'] . '" class="btn btn-sm btn-primary-soft mb-0">Pesan Sekarang</a>';
                                        } ?>
                                    </div>
                                </div>
                                <!-- Card END -->
